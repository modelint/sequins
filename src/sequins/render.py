...
def _draw_threads(layer, diagram: CurtainDiagram) -> None:
    for thread in diagram.threads:
        ...
        # The label hugs the destination String: its edge nearest the target sits the
        # configurable `target string label gap` from the String, on the side the thread
        # arrives from, and the block rides just above the line.
        target_gap = diagram.theme.layout.target_string_label_gap
        label = [thread.label]
        ...
        if going_right:  # target on the right; label to its left, right edge near the target
            ll_x = thread.to_point.x - target_gap - block.width
        else:            # target on the left; label to its right, left edge near the target
            ll_x = thread.to_point.x + target_gap
        TextElement.pin_block(
            layer,
            asset="message",
            text=label,
